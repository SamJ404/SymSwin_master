# light version of SymSwin
# strongly following SRFormer(SRFormer  ICCV2023  Permuted Self-Attention for Single Image Super-Resolution(https://github.com/HVision-NKU/SRFormer))
# modified by SAM J
...
class SyMWB(nn.Module):
    ...
    def __init__(self,
        ...
        self.mlp = DCFF(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class CRAA(nn.Module):
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.norm_pre = norm_layer(2*dim)
        self.norm1 = norm_layer(dim)
        ...
        self.norm_k = norm_layer(2*dim)
        self.k = nn.Sequential(
            nn.Conv2d(2*dim, 2*dim, 1),
            nn.Conv2d(2*dim, 2*dim, 3, 1, 1, groups=dim)
        )
        self.norm_v = norm_layer(2*dim)
        ...
        self.conv_last = nn.Conv2d(dim, dim, 1)
    def forward(self, x_pre, x, x_size):
        B, N, C = x.shape
        H, W = x_size
        shortcut = x.reshape(B, C, H, W)
        x_cross = self.norm_pre(torch.cat([x_pre, x], dim=-1))
        x = self.norm1(x)
        q = self.q(self.norm_q(x).reshape(B, C, H, W)).view(B, C, -1)
        k = self.k(self.norm_k(x_cross).reshape(B, 2*C, H, W)).view(B, 2*C, -1)
        v = self.v(self.norm_v(x_cross).reshape(B, 2*C, H, W)).view(B, 2*C, -1)
        attn = self.softmax(self.act(q @ k.transpose(-2, -1)))
        x = (attn @ v).reshape(B, C, H, W)+shortcut

        shortcut = x
        x = self.norm2(x.reshape(B,N,C)).reshape(B, C, H ,W)
        ff1 = self.ff1(x)
        ff2 = self.ff2(x)
        x = ff1*ff2
        x = self.conv_last(x)+shortcut
...
class SymG(nn.Module):
    ...
    def __init__(self,
        ...
        self.craa = CRAA(dim=dim, norm_layer=norm_layer)
...
class SymGB(nn.Module):
    ...
    def __init__(self,
        ...
        super(SymGB, self).__init__()

        self.dim = dim
        self.input_resolution = input_resolution

        self.residual_group = SymG(
            dim=dim,
            input_resolution=input_resolution,
            depth=depth,
            num_heads=num_heads,
...
class SymSwinLight(nn.Module):
    r""" SymSwin:A Transformer-based Super-resolution Model Focusing on Multi-scale Context for Remote-sensing Images.
    ...
    def __init__(self,
                 ...
                 resi_connection='1conv',
                 **kwargs):
        super(SymSwinLight, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
    ...
    def check_image_size(self, x):
        ...
        for ws in self.window_size:
            mod_pad_h_list.append((ws - h % ws) % ws)
            mod_pad_w_list.append((ws - w % ws) % ws)
        mod_pad_h = max(mod_pad_h_list)
        mod_pad_w = max(mod_pad_w_list)
